...
Currently, only bitmap fonts are supported: 256 character images in a single BMP or PNG, in ASCII order.

h3. Tea::Font.new(path, type[, {:transparent_color => Tea::Color::MAGENTA}])

Load a font from a file.  For now, type must be Tea::Font::BITMAP_FONT.  Setting transparent_color will cause that color to be treated as transparent when the font is loaded.
...
h3. Tea::Font#draw(dest_blittable, x, y, string)
